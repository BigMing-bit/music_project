...
public class SingerServiceImpl extends ServiceImpl<SingerMapper, Singer> implements SingerService {
    ...
    @Override
    public CursorPageResult<SingerVo> getHotSingersCursor(String cursor, Integer size) {
        LambdaQueryWrapper<Singer> qw = new LambdaQueryWrapper<>();
        qw.eq(Singer::getStatus, 1);

        if (StrUtil.isNotBlank(cursor)) {
            qw.lt(Singer::getId, Long.parseLong(cursor));
        }

        qw.orderByDesc(Singer::getId).last("LIMIT " + size);

        List<Singer> singers = this.list(qw);

        CursorPageResult<SingerVo> result = new CursorPageResult<>();
        ...
        List<SingerVo> voList = singers.stream().map(this::toVo).toList();
        result.setList(voList);
        result.setNextCursor(String.valueOf(singers.get(singers.size() - 1).getId()));
        return result;
    }

    @Override
    public CursorPageResult<SingerVo> getNewSingersCursor(String cursor, Integer size) {
        LambdaQueryWrapper<Singer> qw = new LambdaQueryWrapper<>();
        qw.eq(Singer::getStatus, 1);
        ...
        if (StrUtil.isNotBlank(cursor)) {
            ...
            if (arr.length == 2) {
                LocalDateTime cursorTime = LocalDateTime.parse(arr[0]);
                Long cursorId = Long.parseLong(arr[1]);

                qw.and(w -> w.lt(Singer::getCreateTime, cursorTime)
                        .or()
                        .eq(Singer::getCreateTime, cursorTime)
                        .lt(Singer::getId, cursorId));
            }
        }

        qw.orderByDesc(Singer::getCreateTime)
        ...
        Singer last = singers.get(singers.size() - 1);
        result.setNextCursor(last.getCreateTime().toString() + "_" + last.getId());
        return result;
    }

    private SingerVo toVo(Singer s) {
